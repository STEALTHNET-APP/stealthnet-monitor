...
Начиная с v0.1.6 скрипты `make update` и `make rollback` записывают операции в `.state/updates/history.json` в каталоге установки. Каталог сохраняется между Git-релизами и подключается к API только для чтения. Страница **Настройки → Обновления** показывает версии, время, этапы, резервную копию и результат; обновляется каждые 3 секунды.

Хранятся последние 100 операций. В журнал не копируются `.env`, токены и вывод команд. Просмотр доступен после входа в панель. Отдельные этапы прошлых релизов, которые ещё не вели журнал, автоматически восстановить нельзя. Ошибка сборки и результат автоматического отката отражаются отдельно; при новом запуске незавершённая предыдущая операция помечается прерванной.
